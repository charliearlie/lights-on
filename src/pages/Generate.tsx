...
export function Generate() {
  const [imageMap, setImageMap] = useState<ImageMap>(() => {
    ...
    for (const product of products) {
      const off = loadImage(product.id, "off");
      const on = loadImage(product.id, "on");
      if (off || on) {
  ...
  const generateForProduct = async (product: Product) => {
    ...
    try {
      ...
      try {
        if (result.off) saveImage(product.id, "off", result.off);
        if (result.on) saveImage(product.id, "on", result.on);
      } catch {
