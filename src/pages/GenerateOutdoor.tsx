...
interface OutdoorImages {
  off: string | null;
  on: string | null;
}

type ImageMap = Record<number, OutdoorImages>;
...
async function generateOutdoorPair(
  product: Product,
  log?: (msg: string) => void,
): Promise<{ off: string | null; on: string | null }> {
  // Step 1: Generate the outdoor light in its OFF state (daytime, unlit)
  log?.(`Step 1: Requesting OFF image for "${product.name}"...`);
  const offResponse = await ai().models.generateContent({
    model: "gemini-3.1-flash-image-preview",
    contents: `Outdoor product photography of a Scandinavian-designed ${product.description}.
The light is OFF and UNLIT — no glow, no illumination from the fixture.
Photographed during golden hour daylight in a Nordic garden/pathway setting.
Show the craftsmanship, materials, and finish in natural light.
Surrounding context: cobblestone path, trimmed hedges, birch trees, or stone wall.
Style: Architectural outdoor photography, Scandinavian design catalog, photorealistic.
Square format. No people, text, watermarks, or labels.`,
    config: {
  ...
  // Step 2: Send the OFF image back and ask for the ON version at dusk/night
    ...
    contents: [
      {
        ...
        parts: [
          {
            text: `This is a photo of a Scandinavian-designed outdoor light fixture, shown during the day with the light OFF.
Generate a new image of this EXACT SAME fixture and setting, but now at DUSK/NIGHT with the light ON.
Keep the fixture, position, and surrounding environment identical.
Changes:
- The light fixture is ON, casting a warm pool of light
- Show the light pattern, throw distance, and beam shape on surrounding surfaces
- The sky is deep blue dusk or dark night
- Warm amber/white light from the fixture is the primary light source
- Surrounding surfaces (path, wall, foliage) are softly illuminated
- Atmospheric, inviting, Nordic evening ambiance
Style: Architectural outdoor photography, dramatic warm lighting, photorealistic.
Square format. No people, text, watermarks, or labels.`,
          },
          {
            inlineData: {
...
export function GenerateOutdoor() {
  const [imageMap, setImageMap] = useState<ImageMap>(() => {
    ...
    for (const product of outdoor) {
      const off = loadImage("outdoor", product.id, "off");
      const on = loadImage("outdoor", product.id, "on");
      if (off || on) {
  ...
  const generateForProduct = async (product: Product) => {
    ...
    try {
      const result = await generateOutdoorPair(product, (msg) =>
      ...
      try {
        if (result.off) saveImage("outdoor", product.id, "off", result.off);
        if (result.on) saveImage("outdoor", product.id, "on", result.on);
      } catch {
      ...
      <header className="sticky top-0 z-10 border-b border-gray-200 bg-white px-4 py-3 dark:border-gray-700 dark:bg-gray-800">
        <div className="mx-auto flex max-w-7xl items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            >
              &larr; Back to Outdoor
            </Link>
            <h1 className="text-xl font-black text-gray-900 dark:text-white">
              Outdoor Light Image Generator
            </h1>
          </div>
          <button
            onClick={generateAll}
