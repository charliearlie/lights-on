...
export default function OutdoorDetailPage() {
  const { isDark } = useDarkMode();
  const { id } = useParams();
  const productId = Number(id);
  const product = outdoor.find((p) => p.id === productId);
  ...
  const related = getRelated(product, outdoor);
      ...
      <main>
          ...
          <div className="w-full pt-8 lg:w-[48%] lg:pt-16">
            ...
            <div className="flex items-center gap-2.5">
              ...
              <span className="text-[0.75rem] font-medium uppercase tracking-[0.12em] text-[#78716C] dark:text-[#A8A097]">
                {isDark ? "Lit \u2014 casting warm glow" : "Unlit \u2014 toggle lights above"}
              </span>
            </div>
          </div>
        </div>
        ...
        <div className="border-t border-border-light dark:border-border-dark">
          <div className="mx-auto max-w-6xl px-4 pb-14 pt-10 sm:px-6 lg:px-8">
            <p className="mb-5 text-[0.625rem] font-medium uppercase tracking-[0.2em] text-[#78716C] dark:text-[#A8A097]">More outdoor lights</p>
            <div className="flex gap-4 overflow-x-auto pb-2 sm:grid sm:grid-cols-3 sm:gap-5 sm:overflow-visible sm:pb-0">
              {related.map((p, i) => (
                <div key={p.id} className="min-w-[220px] sm:min-w-0">
                  <ProductCard product={p} index={i} linkPrefix="outdoor" />
                </div>
              ))}
            </div>
          </div>
